...
class RegisterController
{
    ...
    {
        $url = "municipalities?select=id_municipality,name_municipality&linkTo=id_department_municipality&equalTo=" . $this->idDptoPqr;
        $method = "GET";
        $fields = array();
...
if (isset($_POST["newPqr"])) {
    $ajax = new RegisterController();
    $ajax->newSupport = $_POST["newPqr"];
...
if (isset($_POST["idDptoPqr"])) {
    $ajax = new RegisterController();
    $ajax->idDptoSupport = $_POST["idDptoPqr"];
    $ajax->edReg = $_POST["edReg"];
    $ajax->mnSelected = $_POST["mnSelected"];
    $ajax->selMunis();
}
