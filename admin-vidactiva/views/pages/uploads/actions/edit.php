...
if (isset($routesArray[3])) {
    ...
    if ($security[1] == $_SESSION["user"]->token_user) {
        ...
        if ($response->status == 200) {
            ...
            /* Busco el registro de validacion */
            $select = "id_validation,id_subject_validation";
            $url = "validations?select=" . $select . "&linkTo=id_subject_validation&equalTo=" . $subjects->id_subject;
            $method = "GET";
            $fields = array();
            $response = CurlController::request($url, $method, $fields);
            if ($response->status == 200) {
                $validations = $response->results[0];
                $idValidation = $validations->id_validation;
            } else {
                $idValidation = "";
            }
        } else {
...
<div class="card card-dark card-outline">
    <form method="post" class="needs-validation" novalidate enctype="multipart/form-data">
        ...
        <input type="hidden" value="<?php echo $idValidation ?>" name="idValidation">
