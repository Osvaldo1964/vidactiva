...
if ($subjects->status == 200) {
    ...
    if ($validations->status == 200) {
        ...
        foreach ($subjects as $subjects) {
            ...
            $aux['subplace_subject'] = $subjects->subplace_subject;
            $aux['valid_subject'] = $subjects->valid_subject;
            $aux['birth_subject'] = $subjects->birth_subject;
            $aux['edad'] = $stredad;
            $aux['sexo'] = $subjects->sex_subject;
            $aux['eps'] = $subjects->eps_subject;
            $aux['afp'] = $subjects->afp_subject;
            $aux['arl'] = $subjects->arl_subject;
    ...
    if (empty($subjectsArray)) {
        ...
        $sheet->setCellValue('B2', 'ROL/CARGO');
        $sheet->setCellValue('C2', 'APELLIDOS Y NOMBRES');
        $sheet->setCellValue('D2', 'TIPO DE DOCUMENTO');
        $sheet->setCellValue('E2', 'NUMERO DE DOCUMENTO');
        $sheet->setCellValue('F2', 'FECHA DE NACIMIENTO');
        $sheet->setCellValue('G2', 'EDAD');
        $sheet->setCellValue('H2', 'SEXO');
        $sheet->setCellValue('I2', 'TELEFONO');
        $sheet->setCellValue('J2', 'CORREO');
        $sheet->setCellValue('K2', 'DIRECCION');
        $sheet->setCellValue('L2', 'DEPARTAMENTO');
        $sheet->setCellValue('M2', 'MUNICIPIO');
        $sheet->setCellValue('N2', 'E.P.S.');
        $sheet->setCellValue('O2', 'A.F.P.');
        $sheet->setCellValue('P2', 'A.R.L.');

               
        $spreadsheet->getActiveSheet()->mergeCells('A1:P1');
        $spreadsheet->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
        $spreadsheet->getActiveSheet()->getStyle('A2:P2')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
        ...
        for ($regj = 0; $regj <= count($subjectsArray) - 1; $regj++) {
            ...
            $sheet->setCellValue("B{$fila}", $subjectsArray[$regj]['subplace_subject']);
            $sheet->setCellValue("C{$fila}", strtoupper($subjectsArray[$regj]['lastname_subject'] . " " . $subjectsArray[$regj]['surname_subject'] . " " .
                $subjectsArray[$regj]['firstname_subject'] . " " . $subjectsArray[$regj]['secondname_subject']));
            $sheet->setCellValue("D{$fila}", $subjectsArray[$regj]['typedoc_subject']);
            $sheet->setCellValue("E{$fila}", $subjectsArray[$regj]['document_subject']);
            $sheet->setCellValue("F{$fila}", $subjectsArray[$regj]['birth_subject']);
            $sheet->setCellValue("G{$fila}", $subjectsArray[$regj]['edad']);
            $sheet->setCellValue("H{$fila}", $subjectsArray[$regj]['sexo']);
            $sheet->setCellValue("I{$fila}", $subjectsArray[$regj]['phone_subject']);
            $sheet->setCellValue("J{$fila}", $subjectsArray[$regj]['email_subject']);
            $sheet->setCellValue("K{$fila}", $subjectsArray[$regj]['address_subject']);
            $sheet->setCellValue("L{$fila}", $subjectsArray[$regj]['name_department']);
            $sheet->setCellValue("M{$fila}", $subjectsArray[$regj]['name_municipality']);
            $sheet->setCellValue("N{$fila}", $subjectsArray[$regj]['eps']);
            $sheet->setCellValue("O{$fila}", $subjectsArray[$regj]['afp']);
            $sheet->setCellValue("P{$fila}", $subjectsArray[$regj]['arl']);
            $fila++;
        }
